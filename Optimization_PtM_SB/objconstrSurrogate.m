function f = objconstrSurrogate(x,len,mdl,power,param)
...
f.Ineq(1:len) = powerIn(:) + powerRequired(:)*0.001 - powerScaled(:);
f.Ineq(len+1:2*len) = param.tankH2LowerBound - tankH2Pressure(:); 
f.Ineq(2*len+1:3*len) = tankH2Pressure(:) - param.tankH2UpperBound;
f.Ineq(3*len+1:4*len) = param.tankMEOHLowerBound - tankMEOHPressure(:);
f.Ineq(4*len+1:5*len) = tankMEOHPressure(:) - param.tankMEOHUpperBound;
